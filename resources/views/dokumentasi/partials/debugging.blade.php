    ...
    <div class="card" style="padding:24px;">
        ...
        <div style="display:flex; flex-direction:column; gap:12px;">
            ...
            $bugs = [
                ...
                [
                    'no' => 7,
                    'error' => 'Login gagal: "Username atau password salah, atau akun tidak aktif" padahal kredensial benar',
                    'lokasi' => 'resources/views/auth/login.blade.php, app/Http/Controllers/AuthController.php',
                    'penyebab' => 'Tabel tb_user kosong (0 rows) karena DatabaseSeeder belum pernah dijalankan setelah migrate, sehingga tidak ada data user sama sekali di database.',
                    'solusi' => 'Menjalankan php artisan db:seed untuk mengisi data awal user (admin, petugas, owner) beserta tarif dan area parkir default.',
                    'status' => 'fixed',
                ],
                [
                    'no' => 8,
                    'error' => 'Role user admin berubah menjadi owner setelah diedit melalui halaman Manajemen User',
                    'lokasi' => 'Database tb_user, admin/user/edit',
                    'penyebab' => 'Admin secara tidak sengaja mengubah role saat mengedit data user lain melalui form Edit User yang memiliki pilihan role.',
                    'solusi' => 'Mengembalikan role ke admin melalui query MySQL langsung: UPDATE tb_user SET role=\'admin\' WHERE username=\'admin\'.',
                    'status' => 'fixed',
                ],
                [
                    'no' => 9,
                    'error' => 'Waktu transaksi tidak sesuai dengan waktu Indonesia (selisih 7 jam)',
                    'lokasi' => 'config/app.php, .env',
                    'penyebab' => 'Timezone aplikasi masih menggunakan UTC (default Laravel), sehingga semua timestamp yang dihasilkan now() berbeda 7 jam dari WIB.',
                    'solusi' => 'Mengubah timezone di config/app.php menjadi Asia/Jakarta dan menambahkan APP_TIMEZONE=Asia/Jakarta di .env, kemudian menjalankan php artisan config:clear.',
                    'status' => 'fixed',
                ],
                [
                    'no' => 10,
                    'error' => 'Halaman form edit kendaraan dan area parkir tidak memiliki desain (tampil sebagai HTML polos)',
                    'lokasi' => 'resources/views/admin/kendaraan/edit.blade.php, resources/views/admin/area/edit.blade.php',
                    'penyebab' => 'View masih menggunakan komponen x-form-card lama dengan class Tailwind yang tidak ter-load, sehingga tampilan tidak memiliki styling apapun.',
                    'solusi' => 'Mengganti seluruh view form dengan desain menggunakan inline CSS yang konsisten dengan halaman lain, termasuk card centered dengan max-width 520px dan margin:0 auto.',
                    'status' => 'fixed',
                ],
                [
                    'no' => 11,
                    'error' => 'Posisi form card tidak konsisten — sebagian di tengah, sebagian di pojok kiri atas',
                    'lokasi' => 'resources/views/admin/area/edit.blade.php, resources/views/admin/tarif/edit.blade.php, resources/views/admin/user/edit.blade.php',
                    'penyebab' => 'Beberapa view menggunakan max-width tanpa margin:0 auto, dan sebagian masih menggunakan komponen x-form-card yang tidak memiliki centering.',
                    'solusi' => 'Menambahkan margin:0 auto pada semua wrapper div form dan mengganti x-form-card dengan div card biasa agar konsisten di semua halaman.',
                    'status' => 'fixed',
                ],
                [
                    'no' => 12,
                    'error' => 'Riwayat transaksi hanya bisa dilihat untuk hari ini, tidak bisa filter tanggal sebelumnya',
                    'lokasi' => 'app/Http/Controllers/TransaksiController.php - method index(), resources/views/petugas/transaksi/index.blade.php',
                    'penyebab' => 'Halaman transaksi tidak memiliki filter tanggal, sehingga petugas tidak bisa mencari transaksi dari hari-hari sebelumnya.',
                    'solusi' => 'Menambahkan filter dari dan sampai (date range) pada form pencarian di view, serta menambahkan kondisi whereDate pada query di controller.',
                    'status' => 'fixed',
                ],
            ];
            @endphp
